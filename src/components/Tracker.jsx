...
const Tracker = () => {
    const [Study, setStudy] = useLocalStorage('tracker_study', 0)
    const [Lazy, setLazy] = useLocalStorage('tracker_lazy', 0)
    const [history, setHistory] = useLocalStorage('tracker_history', [])
    ...
    const showResult = () => {
        if (Study === 0 && Lazy === 0) {
            showToast("No session recorded yet. Start tracking!", 'info');
            return;
        }
        if (Study > Lazy) {
            showToast("Good Job ! You focused More Today ! ", 'success');
        }
        else if (Study === Lazy) {
            showToast("Balanced Day , Not Bad!", 'warning');
        }
        else {
            showToast("Go study Bro! Too Much Procrastination ", 'error');
        }
    }
    ...
    const resetDay = () => {
        ...
        const newEntry = { date: today, study: Study, lazy: Lazy };
        ...
        setMode(null);
        setpaused(false);
        showToast("Day saved & reset! 🔄", 'info');
        ...
        <>
            <div className=' flex justify-center items-center flex-col  px-4 sm:px-8 md:px-16 lg:px-32 py-6 sm:py-8 mx-2 sm:mx-4 gap-2  '>
                ...
                <div className=' flex flex-wrap gap-3 sm:gap-6 justify-center'>
                    ...
                    <button onClick={resetDay} className='border border-gray-400 mt-6 sm:mt-8 mb-4 px-4 sm:px-8 py-2 rounded-md cursor-pointer font-semibold text-sm sm:text-base'>Reset Day</button>
                ...
                {history.length > 0 && (
                    <button
                        ...
                        className='text-xs text-gray-400 underline mt-1 cursor-pointer hover:text-red-400 transition-colors'
                    >
                        Clear history
                    </button>
                )}
